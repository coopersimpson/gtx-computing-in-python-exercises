...
def multiply_file_by_index(filename_write, filename_read):
    input_file = open(filename_read, "r")
    output_file = open(filename_write, "w")
    line_count = 1
    for line in input_file:
        output_file.write(str(int(line) * line_count) + "\n")
        line_count += 1
    input_file.close()
    output_file.close()
