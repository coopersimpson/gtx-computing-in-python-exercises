...
def length_words(a_string):
    ...
    # Create empty dicts with required keys
    output_dict = {}

    for a_string in string_list:
        word_length = len(a_string)
        output_dict[word_length] = []
    
    # Append values to keys of dict
    for a_string in string_list:
        word_length = len(a_string)
        output_dict[word_length].append(a_string)
    return output_dict
