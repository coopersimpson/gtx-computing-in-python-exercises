...
def total_stats(list_of_dicts):
    total_stats = 0
    output_dict = {}
    for dict in list_of_dicts:
        total_stats = dict["hp"] + dict["attack"] + dict["defense"] + dict["special attack"] + dict["special defense"] + dict["speed"]
        output_dict.update({dict["name"] : total_stats})
    return output_dict
