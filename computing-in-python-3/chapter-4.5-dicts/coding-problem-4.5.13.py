...
def word_lengths(a_string):
    ...
    # Clean string
    for char in string.punctuation: # loop over each punctuation char
        a_string = a_string.replace(char, '') # replace any punctuation with nothing
    ...
    for a_string in string_list:
        num_list.append(len(a_string))

    # Create dict using both lists
    for i in range(0, len(num_list)):
        output_dict.update({string_list[i] : num_list[i]})
    return output_dict
