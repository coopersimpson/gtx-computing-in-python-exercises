...
def format_checker(filename):
    ...
    for line in input_file:
        ...
        if len(splitline) == 5:
            check1 = True
        try:
            if isinstance(int(splitline[0]), int) == True and isinstance(int(splitline[2]), int) == True and isinstance(int(splitline[3]), int) == True and isinstance(float(splitline[4]), float):
                check2 = True
        except:
            input_file.close()
            return False
        total_check += float(splitline[4])
    if total_check == 1:
        check3 = True
