...
def find_net_force(list_2tuple_mag_angle):
    #magnitude then angle for each tuple in the list
    hor_total = 0
    ver_total = 0
    for tuple in list_2tuple_mag_angle:
        angle_in_radians = radians(tuple[1])
        hor_component = tuple[0] * cos(angle_in_radians)
        ver_component = tuple[0] * sin(angle_in_radians)
        hor_total += hor_component
        ver_total += ver_component
    total_magnitude = sqrt(hor_total ** 2 + ver_total ** 2)
    angle = atan2(ver_total, hor_total)
    angle_in_degrees = degrees(angle)
    return (round(total_magnitude, 1), round(angle_in_degrees, 1))
