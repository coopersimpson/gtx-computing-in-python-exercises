...
def find_net_force(list_forces):
    sum_vert = 0
    sum_horz = 0
    for force in list_forces:
        sum_vert += force.get_vertical()
        sum_horz += force.get_horizontal()
    net_mag = round(((sum_horz ** 2 + sum_vert ** 2) ** 0.5), 1)
    net_angle = round(degrees(atan2(sum_vert, sum_horz)), 1)
    return Force(net_mag, net_angle)
